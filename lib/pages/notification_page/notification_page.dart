...
class CustomAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  final Function()? onSearchPressed; // Funktion för att hantera sökevenemang
  ...
  const CustomAppBar({
    ...
    this.onSearchPressed, // Lägg till den nya funktionen för sökning
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Container(
        ...
        child: AppBar(
          automaticallyImplyLeading: false, // Tar bort standardtillbakaknappen från appbaren
          elevation: 0,
          centerTitle: true, // Centrera titeln
          title: Row(
            children: [
              // Tillbaka-pilen till vänster om titeln
              IconButton(
                icon: const Icon(
                  Icons.arrow_back,
              ...
              // Sökikonen till höger om titeln
              IconButton(
                icon: const Icon(
                  Icons.search,
                  color: Color(0xFF4F4F4F),
                ),
                onPressed: onSearchPressed, // Anropa funktionen för sökevenemang
              ),
              ...
              if (useActionButton)
                if (actionIcon != null && onActionPressed != null)
                  IconButton(
                    icon: Icon(
                      actionIcon,
                      color: actionIcon != null ? Color(0xFF4F4F4F) : Colors.transparent,
                    ),
                    onPressed: onActionPressed,
                  ),
              // Om ActionButton inte ska användas, gör den osynlig
              if (!useActionButton)
                IconButton(
                  icon: Icon(
                    actionIcon,
                    color: actionIcon != null ? Color.fromARGB(255, 255, 255, 255) : Color.fromARGB(255, 255, 255, 255),
                  ),
                  onPressed: onActionPressed,
                ),
            ],
  ...
  // Appbar klassen kräver preferredSize att sätta
  @override
